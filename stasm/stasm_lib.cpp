    ...
    float* estyaw)         // out: NULL or pointer to estimated yaw
{
    ...
    *foundface = 0;        // but assume no face found
    ...
    try
    {
        ...
        {
			/*cv::Mat newimg = img_g.clone();
			cv::Rect rect;
			rect.x = detpar_roi.x - detpar_roi.width/2;
			rect.y = detpar_roi.y - detpar_roi.height/2;
			rect.width = detpar_roi.width;
			rect.height = detpar_roi.height;
			rectangle(newimg, rect, cv::Scalar(0, 0, 0));
