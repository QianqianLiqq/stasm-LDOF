...
vec_Rect Detect(                            // detect faces or facial features
    ...
    const Rect*            searchrect,      // in: search in this region, can be NULL    null
    double                 scale_factor,    // in   1.1 
    int                    min_neighbors,   // in   3
    int                    flags,           // in   0
    int                    minwidth_pixels) // in: reduces false positives
{
    ...
    Image roi(img,
              searchrect1.width? searchrect1: Rect(0, 0, img.cols, img.rows));//全图搜索脸
    ...
    if (!feats.empty() && searchrect1.width)//如果给定了搜索区域，则要将找到的人脸矩形框恢复到原图中
