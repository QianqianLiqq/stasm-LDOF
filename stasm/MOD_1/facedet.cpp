    ...
    const Image& img)        // io
{
    ...
    copyMakeBorder(bordered_img, bordered_img,
                   ...
                   cv::BORDER_REPLICATE);
    return bordered_img;//bordered_img为img上下同时加宽BORDER_FRAC * bordered_img.rows，左右同时加宽BORDER_FRAC * bordered_img.cols
}

void DetectFaces(          // all face rects into detpars
    vec_DetPar&  detpars,  // out
    const Image& img,      // in
    int          minwidth) // in: as percent of img width   10
{
	...
	//跟踪人脸
	/*vec_Rect facerects;
	faceBox.x += leftborder;
	faceBox.y += topborder;
	facerects.push_back(faceBox);*/
    ...
    for (int i = 0; i < NSIZE(facerects); i++)
    {
		...
		//std::cout << facerects[i].x << " " << facerects[i].y << std::endl;
        ...
        detpar.y -= topborder;         //detpar的x和y为检测到的脸的中心
    ...
    vec_DetPar& detpars) // io
{
    ...
    if (NSIZE(detpars) >= 3) // need at least 3 faces
    {
        ...
        const int median     = cvRound(detpars[NSIZE(detpars) / 2].width);  //所有检测到的脸矩形框的median大小
        const int minallowed = cvRound(median / MIN_WIDTH); //允许的最大矩形框
        const int maxallowed = cvRound(MAX_WIDTH * median); //允许的最小矩形框
...
{
    CV_Assert(user == NULL);
    DetectFaces(detpars_, img, minwidth);   //输出detpars_，为图片中检测到的脸的一些参数
    char tracepath[SLEN];
    sprintf(tracepath, "%s_00_unsortedfacedet.bmp", Base(imgpath));
	//printf(tracepath);
    TraceFaces(detpars_, img, tracepath);   //没起任何作用
    DiscardMissizedFaces(detpars_);  //舍弃太大或太小的矩形框
    ...
    { 
		...
		faceCMT.x = detpars_[0].x - detpars_[0].width / 2;
		faceCMT.y = detpars_[0].y - detpars_[0].height / 2;
		faceCMT.width = detpars_[0].width;
		faceCMT.height = detpars_[0].height;

		if (firstFace.width == 0)
			firstFace = faceCMT;
		else{
			faceCMT.x = detpars_[0].x - firstFace.width / 2;
			faceCMT.y = detpars_[0].y - firstFace.height / 2;
			faceCMT.width = firstFace.width;
			faceCMT.height = firstFace.height;
		}
    }
