    ...
    const Rect&   rect_roi)        // in
{
    ...
    if (Valid(detpar_roi.lex))//detpar_roi.lex=INVALID
    {
		//IX=0,IY=1
        eyemouth_shape(0, IX) -= rect_roi.x; 
        eyemouth_shape(0, IY) -= rect_roi.y;
    }
    if (Valid(detpar_roi.rex))//detpar_roi.rex=INVALID
    {
        eyemouth_shape(1, IX) -= rect_roi.x;
        eyemouth_shape(1, IY) -= rect_roi.y;
    }
    if (Valid(detpar_roi.mouthx))//detpar_roi.mouthx=INVALID
    {
        eyemouth_shape(2, IX) -= rect_roi.x;
        eyemouth_shape(2, IY) -= rect_roi.y;
    }
    if (Valid(detpar.rot) && detpar.rot)//INVALID
    {
    ...
    double        rightfrac)  // in
{
    ...
    detpar_roi = ImgDetParToRoiFrame(detpar, rect_roi);//detpar_roi=detpar

    // following "if"s are for efficiency (avoid rotation etc. when possible).

    if (detpar.rot == 0 && IsRoiEntireImg(rect_roi, img.cols, img.rows))   //false
        face_roi = img;

	else if (!Valid(detpar.rot) || detpar.rot == 0)  { //true
		face_roi = Image(img, rect_roi);
		for (int i = 0; i < initialPoints.size(); ++i){
			initialPoints[i].x -= rect_roi.x;
			initialPoints[i].y -= rect_roi.y;
			//cv::rectangle(face_roi, cv::Rect(initialPoints[i].x, initialPoints[i].y, 100, 100), cv::Scalar(0, 0, 0));
		}
	}
