    ...
    const MAT&   covi)     // in: inverse of the covar of the training profiles
{
	...
	if (ipoint == 59){
		std::cout << "beforeclass" << x << " " << y << std::endl;
		x = (double)initialPoints[0].x * img.cols / scaledimgCols;
		y = (double)initialPoints[0].y * img.rows / scaledimgRows;
		std::cout << "afterclass" << x << " " << y << std::endl;
	}
	else if (ipoint == 62){
		x = (double)initialPoints[1].x * img.cols / scaledimgCols;
		y = (double)initialPoints[1].y * img.rows / scaledimgRows;
	}
	else if (ipoint == 65){
		x = (double)initialPoints[2].x * img.cols / scaledimgCols;
		y = (double)initialPoints[2].y * img.rows / scaledimgRows;
	}
	else if (ipoint == 74){
		x = (double)initialPoints[3].x * img.cols / scaledimgCols;
		y = (double)initialPoints[3].y * img.rows / scaledimgRows;
	}
